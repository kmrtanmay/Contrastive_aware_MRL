...
from utils.distributed_gpu import (
    setup,
    cleanup,
    get_device,
    wrap_ddp_model,
...
def train_on_gpu(rank, world_size, args):
    """
    Training function to be run on each GPU
    ...
    try:
        ...
        model_q = wrap_ddp_model(model_q, device_id=rank)
        ...
        # Define optimizer (scale learning rate by world_size)
...
def parse_args():
    parser = argparse.ArgumentParser(description='Train MoCo Matryoshka on ImageNet-100 with Multi-GPU')
    ...
    parser.add_argument('--log-dir', type=str, default='runs/matryoshka_moco_multigpu',
                        help='tensorboard log directory (default: runs/matryoshka_moco_multigpu)')
    parser.add_argument('--checkpoint-dir', type=str, default='../checkpoints/moco_mrl',
                        help='checkpoint directory (default: ../checkpoints/moco_mrl)')
...
def main():
    ...
    # Use the specified number of GPUs or all available
